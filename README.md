...
StageCounter is a count-up timer with configurable start and end time.
...
* **Address**
  ...
  * `/stagecounter/play` → Play / Pause toggle
  * `/stagecounter/stop` → Stop / Reset
...
StageCounter는 시작시간/종료시간 설정이 가능한 카운트업 타이머 입니다.
...
* **주소**
  ...
  * `/stagecounter/play` → 재생 / 일시정지
  * `/stagecounter/stop` → 정지 / 리셋
